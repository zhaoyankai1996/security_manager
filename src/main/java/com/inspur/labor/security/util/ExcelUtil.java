...
public class ExcelUtil {
    ...
    public static MultipartFile urlToMultipartFile(String url, String fileName) {
        ...
        try {
            logger.info("urlToMultipartFile文件转换中，url为：" + url + "文件名称为：" + fileName);
            ...
            logger.error("url解析失败，抛出url解析异常");
        } finally {
            httpUrl.disconnect();
        }
    ...
    public static MultipartFile fileToMultipartFile(File file) {
        ...
        try {
            ...
            while ((bytesRead = fis.read(buffer, 0, Constant.BUFFLENGTH)) != -1) {
                os.write(buffer, 0, bytesRead);
            }
            os.close();
            fis.close();
    ...
    public static FileItem createFileItem(InputStream inputStream, String fileName) {
        ...
        try {
            ...
            while ((bytesRead = inputStream.read(buffer, 0, Constant.BUFFLENGTH)) != -1) {
                os.write(buffer, 0, bytesRead);
            }
            inputStream.close();
        } catch (IOException e) {
